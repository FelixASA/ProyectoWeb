...
export default {
  ...
  methods: {
    initialize() {
      onMounted(async () => {
        const valores = await fetch("http://localhost:4000/arrendatario");
    ...
    async postData(data) {
      const response = await fetch("http://localhost:4000/arrendatario", {
        method: "POST",
        headers: {
    ...
    async deleteData(data){
        const id = data.id
        const response = await fetch("http://localhost:4000/arrendatario/" + id,{
            method: "DELETE",
        })
